...
// lib/util/generator.ts
var generateNewEntryFilePath = ({ entryFilePath }) => {
...
// lib/util/params.ts
var params_default = async () => {
  const argv = await yargs_default(process.argv.slice(2)).option("nginx-path", {
...
var rebuildEntryFile = async ({ entryFilePath, newEntryFilePath }) => {
  await import_promises.default.rename(entryFilePath, newEntryFilePath);
};
var rebuildNginxFile = async ({ params }) => {
...
var readNginxIndexFile = async ({ nginxPath }) => {
  const file = await import_promises.default.readFile(nginxPath);
  return file.toString();
};
var injectEntryFileNameToNginxConf = ({ nginxContent, newEntryFileName }) => {
  return nginxContent.replace(/\${deploy_web_entry}/g, newEntryFileName);
};
var writeNginxConf = async ({ nginxContent, nginxPath }) => {
  const fileContentBuffer = new Uint8Array(import_buffer.Buffer.from(nginxContent));
  await import_promises.default.writeFile(nginxPath, fileContentBuffer);
};

// lib/index.ts
var main = async () => {
  console.log("Starting build web entry.");
  const params = await params_default();
